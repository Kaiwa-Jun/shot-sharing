...
export function PageClient({ initialPhotos, initialUser }: PageClientProps) {
  ...
  // /me画面ではSearchFABを非表示
  const showSearchFAB = pathname === "/test-embedding";
  ...
  const handlePhotoClick = async (
    ...
    if (cachedSimilarPosts) {
      console.log(
        `💾 [Embedding Test] キャッシュから類似作例を取得: ${cachedSimilarPosts.length}件`
      );
    ...
    try {
      console.log(`🔍 [Embedding Test] 投稿詳細データを取得中: ${photoId}`);
      ...
      if (!cachedSimilarPosts) {
        console.log(`🧪 [Embedding Test] Embedding検索を実行中...`);
      ...
      if (!cachedSimilarPosts && similarPostsResult) {
        console.log(`📊 [Embedding Test] 類似作例の取得結果:`, {
      ...
      if (!cachedSimilarPosts && similarPostsResult) {
        if (similarPostsResult.data) {
          ...
          console.log(
            `✅ [Embedding Test] 類似作例を設定してキャッシュに保存: ${similarPostsResult.data.length}件`
          );
        } else {
          setSimilarPosts([]);
          // 空の結果もキャッシュに保存（再取得を防ぐため）
          setSimilarPostsCache((prev) => new Map(prev).set(photoId, []));
          console.log(`⚠️ [Embedding Test] 類似作例なし（キャッシュに保存）`);
        }
      }
    } catch (error) {
  ...
  const handleCloseModal = () => {
    ...
    window.history.replaceState(null, "", "/test-embedding");
  ...
  const handleSimilarPostClick = (postId: string) => {
    console.log(`🎯 [Embedding Test] 類似作例クリック: ${postId}`);
    // 新しい投稿をモーダルで表示
  ...
  const handleSearch = async (query: string) => {
    try {
      ...
      console.log("🔍 [Embedding Test] 検索開始:", query);
        ...
        for (const line of lines) {
          if (line.startsWith("data: ")) {
            ...
            if (data.type === "text") {
              ...
              console.log("✅ [Embedding Test] ストリーミング完了:", {
